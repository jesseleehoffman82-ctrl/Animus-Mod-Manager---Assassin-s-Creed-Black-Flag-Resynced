...
function Apply-HighDetailPng($record,[string]$path) {
    ...
    Send-HighDetailMessage @{type='apply';surfaceId=[string]$record.HDSurfaceId;shared=[bool]$record.HDShared;url=('http://127.0.0.1:8767/ship-preview/'+$name)}
    $record.Texture=$path
    return $true
}
...
function Initialize-HighDetailViewer {
    try {$healthy=(Invoke-WebRequest 'http://127.0.0.1:8767/health' -UseBasicParsing -TimeoutSec 1).Content -eq 'jackdaw-studio-viewer'} catch {$healthy=$false}
    if (-not $healthy) {
        Start-Process -FilePath (Get-Command python.exe).Source -ArgumentList ('"'+(Join-Path $appRoot 'studio_viewer_host.py')+'"') -WorkingDirectory $appRoot -WindowStyle Hidden
    }
    try { $backend=Invoke-WebRequest 'http://127.0.0.1:3001/api/projects' -UseBasicParsing -TimeoutSec 1 } catch {
        $studioRoot=[IO.Path]::GetFullPath((Join-Path $appRoot '..\..\3D Gen Studio\resources\app'))
        Start-Process -FilePath (Get-Command node.exe).Source -ArgumentList ('"'+(Join-Path $studioRoot 'server.js')+'"') -WorkingDirectory $studioRoot -WindowStyle Hidden
    }
    ...
    $script:hdBrowser.Add_CoreWebView2InitializationCompleted({param($sender,$eventArgs)
        ...
        $core.Add_WebMessageReceived({param($sender,$eventArgs)
            if (-not $eventArgs.Source.StartsWith('http://127.0.0.1:8767/')) { return }
            try {
                $m=$eventArgs.WebMessageAsJson | ConvertFrom-Json
                switch ($m.type) {
                    'selftest-pass' { [IO.File]::WriteAllText((Join-Path $dataRoot 'studio-selection-test.json'),($m | ConvertTo-Json -Depth 6)) }
        ...
        $core.Navigate('http://127.0.0.1:8767/mesh-editor?url=%2Fstudio-boot.glb&name=Jackdaw&embedded=1&v='+$viewerRevision)
    })
